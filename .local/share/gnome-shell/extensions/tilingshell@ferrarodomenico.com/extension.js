...
// src/components/tilingsystem/popupWindowPreview.ts
var WINDOW_OVERLAY_FADE_TIME = 200;
var WINDOW_SCALE_TIME = 200;
var WINDOW_ACTIVE_SIZE_INC = 5;
var ICON_SIZE = 36;
var ICON_OVERLAP = 0.7;
var ICON_TITLE_SPACING = 6;
var PopupWindowPreview = class extends Shell.WindowPreview {
  _overlayShown;
  _icon;
  _metaWindow;
  _windowActor;
  ...
  _adjustOverlayOffsets() {
    ...
    const [previewWidth, previewHeight] = this._previewContainer.allocation.get_size();
...
__publicField(PopupWindowPreview, "metaInfo", {
  GTypeName: "PopupWindowPreview"
});
PopupWindowPreview = __decorateClass([
  registerGObjectClass
], PopupWindowPreview);

// src/components/tilepreview/popupTilePreview.ts
var PopupTilePreview = class extends TilePreview {
  _blur;
  constructor(params) {
    super(params);
    this._blur = false;
...
__publicField(PopupTilePreview, "metaInfo", {
...
PopupTilePreview = __decorateClass([
  registerGObjectClass
], PopupTilePreview);

// src/components/tilingsystem/tilingPopup.ts
var debug6 = logger("TilingPopup");
var MASONRY_LAYOUT_SPACING = 32;
var ANIMATION_SPEED = 200;
var MASONRY_ROW_MIN_HEIGHT_PERCENTAGE = 0.3;
var MasonryLayout = class extends Clutter.LayoutManager {
  _rowCount;
  _spacing;
  _maxRowHeight;
  _rowHeight;
  ...
  vfunc_allocate(container, box) {
    ...
    let rowHeight = 0;
    while (this._rowCount > 1 && rowHeight < box.get_height() * MASONRY_ROW_MIN_HEIGHT_PERCENTAGE) {
      this._rowCount--;
      rowHeight = (box.get_height() - this._spacing * (this._rowCount - 1)) / this._rowCount;
    }
    rowHeight = Math.min(rowHeight, this._maxRowHeight);
    rowHeight = this._rowHeight;
    const rowWidths = Array(this._rowCount).fill(0);
    const contentHeight = rowHeight * this._rowCount + this._spacing * (this._rowCount - 1);
    const placements = [];
    ...
    for (const placement of placements) {
      ...
      const y = box.y1 + row * (rowHeight + this._spacing) + verticalOffset;
  ...
  vfunc_get_preferred_width(container, forHeight) {
    ...
    if (children.length === 0)
      return [0, 0];
    const rowWidths = Array(this._rowCount).fill(0);
    const rowWidth = (forHeight - this._spacing * (this._rowCount - 1)) / this._rowCount;
    for (const child of children) {
  ...
  vfunc_get_preferred_height(container, forWidth) {
    ...
    const totalHeight = this._rowCount * maxChildHeights + (this._rowCount - 1) * this._spacing;
    return [totalHeight, totalHeight];
  }
};
MasonryLayout = __decorateClass([
  registerGObjectClass
], MasonryLayout);
var TilingPopup = class extends LayoutWidget {
  _signals;
  _lastTiledWindow;
  _showing;
  constructor(layout, innerGaps, outerGaps, workarea, scalingFactor, window) {
    super({
      containerRect: workarea,
      parent: global.windowGroup,
      layout: new Layout([], ""),
    ...
    this._signals.connect(
      ...
      (_2, event) => {
        ...
        if (!isDescendant || event.get_source() === this || event.get_source().get_layout_manager() instanceof MasonryLayout)
          this.close();
      }
    );
    this._signals.connect(
  ...
  _relayoutVacantTiles(layout, tiledWindows, window) {
    const tiles = layout.tiles;
    const windowDesiredRect = window.assignedTile ? TileUtils.apply_props(window.assignedTile, this._containerRect) : window.get_frame_rect();
    const vacantTiles = tiles.filter((t) => {
      if (window.assignedTile && t.x === window.assignedTile.x && t.y === window.assignedTile.y && t.width === window.assignedTile.width && t.height === window.assignedTile.height)
        return false;
      const tileRect = TileUtils.apply_props(t, this._containerRect);
      return !tiledWindows.find(
        (win) => tileRect.overlap(
  ...
  buildTile(parent, rect, gaps, tile) {
    ...
    const layoutManager10 = new MasonryLayout(
    ...
    const container = new St.Widget({
      ...
      y_expand: true,
      pivot_point: new Graphene.Point({ x: 0.5, y: 0.5 }),
      layout_manager: layoutManager10,
      style: "padding: 32px;"
    });
  ...
  _recursivelyShowPopup(nontiledWindows, monitorIndex) {
    ...
    nontiledWindows.forEach((nonTiledWin) => {
      ...
      winClone.connect("button-press-event", () => {
        ...
        nonTiledWin.originalSize = nonTiledWin.get_frame_rect().copy();
        winClone.opacity = 0;
        preview.ease({
          opacity: 0,
          duration: ANIMATION_SPEED,
        ...
        winActor.ease({
          ...
          onStopped: () => {
            winActor.set_pivot_point(0, 0);
            if (this._previews.length === 0 && this._lastTiledWindow) {
...
var TilingManager = class {
  ...
  _onMovingWindow(window, grabOp) {
    ...
    if (!showTilingSystem) {
      ...
      if (Settings.ACTIVE_SCREEN_EDGES && !this._isSnapAssisting && this._edgeTilingManager.canActivateEdgeTiling(currPointerPos)) {
  ...
  _onWindowGrabEnd(window) {
    ...
    if (!isTilingSystemActivated && !this._isSnapAssisting && !this._edgeTilingManager.isPerformingEdgeTiling())
      return;
    const showPopup = !this._isSnapAssisting && !this._edgeTilingManager.isPerformingEdgeTiling();
    this._isSnapAssisting = false;
  ...
  _isPointerInsideThisMonitor(window) {
    ...
    const monitorWidth = this._workArea.x - this._monitor.x + this._workArea.width;
    const monitorHeight = this._workArea.y - this._monitor.y + this._workArea.height;
    return x >= this._monitor.x && x <= this._monitor.x + monitorWidth && y >= this._monitor.y && y <= this._monitor.y + monitorHeight;
  }
...
var debug8 = logger("WindowBorderManager");
...
var TilingShellExtension = class extends Extension {
  ...
  _validateSettings() {
    if (Settings.LAST_VERSION_NAME_INSTALLED === "14.0") {
      debug9("apply compatibility changes");
  ...
  enable() {
    ...
    debug9("extension is enabled");
  }
  openLayoutEditor() {
    this._indicator?.openLayoutEditor();
  }
  _createTilingManagers() {
    debug9("building a tiling manager for each monitor");
  ...
  _setupSignals() {
    ...
    this._signals.connect(
      ...
      () => {
        ...
        if (Settings.ACTIVE_SCREEN_EDGES) {
          debug9("disable native edge tiling");
          SettingsOverride.get().override(
            gioSettings,
            "edge-tiling",
            new GLib.Variant("b", false)
          ...
          debug9("bring back native edge tiling");
          SettingsOverride.get().restoreKey(
            gioSettings,
            "edge-tiling"
          );
